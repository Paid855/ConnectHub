...
<div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;padding:30px;background:#fff;border-radius:16px;border:1px solid #ffe0ec;">
  ...
  ${cta ? `<div style="text-align:center;padding:16px 0;"><a href="${ctaUrl || 'https://connecthub.com/dashboard'}" style="background:linear-gradient(135deg,#e11d48,#ec4899);color:#fff;padding:12px 32px;border-radius:50px;text-decoration:none;font-weight:bold;font-size:14px;">${cta}</a></div>` : ''}
...
export async function emailNewMessage(toEmail: string, toName: string, fromName: string) {
  return sendEmail(toEmail, `${fromName} sent you a message on ConnectHub`,
    template("New Message!", `<p>Hi ${toName},</p><p><strong>${fromName}</strong> just sent you a message. Don't keep them waiting!</p>`, "Read Message", "https://connecthub.com/dashboard/messages")
  );
}
...
export async function emailNewMatch(toEmail: string, toName: string, fromName: string) {
  return sendEmail(toEmail, `You have a new match on ConnectHub! 💕`,
    template("It's a Match! 💕", `<p>Hi ${toName},</p><p>Great news! <strong>${fromName}</strong> wants to connect with you. Start a conversation now!</p>`, "Say Hello", "https://connecthub.com/dashboard/messages")
  );
}
...
export async function emailFriendRequest(toEmail: string, toName: string, fromName: string) {
  return sendEmail(toEmail, `${fromName} wants to be your friend on ConnectHub`,
    template("New Friend Request!", `<p>Hi ${toName},</p><p><strong>${fromName}</strong> sent you a friend request. Check out their profile and connect!</p>`, "View Request", "https://connecthub.com/dashboard/friends")
  );
}
...
export async function emailGiftReceived(toEmail: string, toName: string, fromName: string, giftEmoji: string, giftName: string) {
  return sendEmail(toEmail, `${fromName} sent you a ${giftEmoji} ${giftName}!`,
    template(`You received a ${giftEmoji} ${giftName}!`, `<p>Hi ${toName},</p><p><strong>${fromName}</strong> sent you a <strong>${giftEmoji} ${giftName}</strong> on ConnectHub! Your coin balance has been updated.</p>`, "View Gifts", "https://connecthub.com/dashboard/coins")
  );
}

export async function emailVerified(toEmail: string, toName: string) {
  return sendEmail(toEmail, `You're verified on ConnectHub! ✅`,
    template("Identity Verified! ✅", `<p>Hi ${toName},</p><p>Congratulations! Your identity has been verified. You now have the verified badge and will get up to 5x more matches!</p>`, "View Profile", "https://connecthub.com/dashboard/profile")
  );
}
...
export async function emailStoryReply(toEmail: string, toName: string, fromName: string) {
  return sendEmail(toEmail, `${fromName} replied to your story`,
    template("Story Reply!", `<p>Hi ${toName},</p><p><strong>${fromName}</strong> replied to your story. Check your messages to see what they said!</p>`, "View Reply", "https://connecthub.com/dashboard/messages")
  );
}
...
export async function emailWelcome(toEmail: string, toName: string) {
  return sendEmail(toEmail, `Welcome to ConnectHub! 💕`,
    ...
    `, "Complete Your Profile", "https://connecthub.com/dashboard/profile")
  );
}
