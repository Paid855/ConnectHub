...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.emailOrUsername || !form.password) { setError("Enter email/username and password"); return; }
    setLoading(true);
    setError("");
    try {
    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex flex-1 bg-gradient-to-br from-rose-500 via-pink-500 to-purple-600 items-center justify-center p-12">
        <div className="max-w-md text-center">
          <img src="/logo.png" alt="ConnectHub" className="w-20 h-20 mx-auto mb-6 rounded-2xl" onError={(e: any) => { e.target.style.display = "none"; }} />
          ...
          <div className="flex items-center justify-center gap-6 mt-10">
            <div className="text-center"><p className="text-2xl font-bold text-white">10K+</p><p className="text-xs text-rose-200">Members</p></div>
            <div className="text-center"><p className="text-2xl font-bold text-white">50K+</p><p className="text-xs text-rose-200">Matches</p></div>
            <div className="text-center"><p className="text-2xl font-bold text-white">4.9</p><p className="text-xs text-rose-200">Rating</p></div>
          </div>
        </div>
      </div>
      <div className="flex-1 flex items-center justify-center p-4 sm:p-12 bg-white">
        <div className="w-full max-w-md">
          <Link href="/" className="flex items-center gap-2 mb-8">
            <img src="/logo.png" alt="ConnectHub" className="w-8 h-8 rounded-lg" onError={(e: any) => { e.target.style.display = "none"; }} />
          ...
          <p className="text-gray-500 text-sm mb-8">Do not have an account? <Link href="/signup" className="text-rose-500 font-semibold hover:underline">Sign up free</Link></p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div>
              <div className="flex justify-between mb-1">
                ...
                <Link href="/forgot-password" className="text-xs text-rose-500 font-medium hover:underline">Forgot password?</Link>
              </div>
              <div className="relative">
