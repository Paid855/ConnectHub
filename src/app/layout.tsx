...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const loadUser = async (attempt = 1) => {
    ...
    try {
      ...
      if (!data.user) {
        // After login, some mobile browsers and Google Translate reloads can be a little slow
        // to expose the fresh auth cookie to /api/auth/me. Do not show a false access error.
        if (attempt < MAX_AUTH_ATTEMPTS) {
          setTimeout(() => loadUser(attempt + 1), RETRY_DELAY_MS);
          return;
        }
