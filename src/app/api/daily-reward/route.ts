...
export async function POST(req: NextRequest) {
  ...
  await prisma.user.update({ where: { id }, data: { coins: { increment: 5 }, lastLoginReward: new Date() } });
  await prisma.coinTransaction.create({ data: { userId: id, amount: 5, type: "daily_reward", description: "Daily login reward" } }).catch(() => {});

  return NextResponse.json({ success: true, reward: 5 });
}
