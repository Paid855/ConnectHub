...
export async function GET(req: NextRequest) {
  ...
  // Get likes I SENT — filter out mutual likes (already matched)
  ...
  // Remove users I already liked back
  ...
  const results = filteredLikes.map(l => {
    ...
    return {
      ...
      user: canSee ? {
        id: liker.id, name: liker.name, profilePhoto: liker.profilePhoto, age: liker.age,
        gender: liker.gender, bio: liker.bio, tier: liker.tier, verified: liker.verified,
        city: liker.city, country: liker.country,
        isOnline: liker.lastActive ? Date.now() - new Date(liker.lastActive).getTime() < 5 * 60 * 1000 : false,
      } : { id: liker.id, name: "???", profilePhoto: null, blurred: true },
    };
  }).filter(Boolean);

  return NextResponse.json({
    likes: results, total: results.length,
