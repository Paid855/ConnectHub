...
export async function GET(req: NextRequest) {
  ...
  const friends = accepted.map(f => {
    const friendUser = getUser(f.userId === id ? f.friendId : f.userId);
    return { id: f.id, user: friendUser, createdAt: f.createdAt };
  }).filter(f => f.user);

  const requests = pending.map(f => ({
    id: f.id, user: getUser(f.userId), createdAt: f.createdAt
  })).filter(r => r.user);

  const sent = sentReqs.map(f => ({
    id: f.id, friend: getUser(f.friendId), createdAt: f.createdAt
  })).filter(s => s.friend);
