...
export async function GET(req: NextRequest) {
  ...
  const referrals = await prisma.user.findMany({
    where: { referredBy: id },
    select: { id:true, name:true, profilePhoto:true, createdAt:true },
...
export async function POST(req: NextRequest) {
  ...
  if (!referralCode) return NextResponse.json({ error: "Enter a code" }, { status: 400 });
  ...
  await prisma.coinTransaction.create({ data: { userId: id, amount: 25, type: "referral_bonus", description: "Referral bonus" } }).catch(() => {});
  await prisma.coinTransaction.create({ data: { userId: referrer.id, amount: 50, type: "referral_reward", description: "Invited a friend" } }).catch(() => {});
