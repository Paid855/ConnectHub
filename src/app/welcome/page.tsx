  ...
  return (
    <div className={"min-h-screen transition-colors duration-500 overflow-hidden " + (dk ? "bg-[#0a0a0c]" : "bg-[#fafafa]")}>
      <style jsx global>{String.raw`
        @import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700;1,400;1,500&family=DM+Sans:wght@300;400;500;600;700&display=swap');
        .font-display { font-family: 'Cormorant Garamond', serif; }
        @keyframes chSplashGlow { 0%,100% { transform: scale(1); opacity: 0.4; } 50% { transform: scale(1.08); opacity: 0.7; } }
        @keyframes chLogoIn { 0% { opacity: 0; transform: scale(0.4) rotate(-10deg); } 60% { transform: scale(1.06) rotate(2deg); } 100% { opacity: 1; transform: scale(1) rotate(0); } }
        @keyframes chFadeUp { from { opacity: 0; transform: translateY(14px); } to { opacity: 1; transform: translateY(0); } }
        @keyframes chLoadBar { from { transform: scaleX(0); } to { transform: scaleX(1); } }
        @keyframes chSpin { to { transform: rotate(360deg); } }
        @keyframes chHeartPulse { 0%,100% { transform: scale(1); } 12% { transform: scale(1.1); } 24% { transform: scale(1); } 36% { transform: scale(1.06); } 48% { transform: scale(1); } }
        @keyframes chFloat { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-16px); } }
      `}</style>
      ...
      <div className={"fixed inset-0 z-50 flex flex-col items-center justify-center transition-all duration-700 " + (splash ? "opacity-100" : "opacity-0 pointer-events-none") + " " + (dk ? "bg-[#0a0a0c]" : "bg-[#fafafa]")}>
        <div className={"absolute w-[350px] h-[350px] rounded-full " + (dk ? "bg-rose-500/10" : "bg-rose-500/5")} style={{animation:"chSplashGlow 3s ease-in-out infinite"}} />
        ...
        <div className={"mt-10 w-9 h-[3px] rounded-full overflow-hidden relative z-10 " + (dk ? "bg-white/8" : "bg-gray-200")} style={{animation:"chFadeUp 0.4s ease 0.9s both"}}>
          <div className="h-full bg-gradient-to-r from-rose-500 to-pink-500 rounded-full" style={{transformOrigin:"left",animation:"chLoadBar 1.6s ease-in-out 1s forwards",transform:"scaleX(0)"}} />
        </div>
      </div>
      ...
      <div className={"min-h-screen flex flex-col transition-all duration-700 " + (splash ? "opacity-0 scale-95" : "opacity-100 scale-100")}>
        ...
        <div className="flex justify-end p-5 pt-14 sm:pt-6">
          <button onClick={() => setTheme(dk ? "light" : "dark")} className={"w-11 h-11 rounded-full flex items-center justify-center text-lg transition-all active:scale-90 " + (dk ? "bg-white/5 border border-white/8 hover:bg-white/10" : "bg-gray-100 border border-gray-200 hover:bg-gray-200")}>
            {dk ? "☀️" : "🌙"}
          </button>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center px-6 relative">
          ...
          <div className="relative w-[250px] h-[250px]">
            ...
            {/* Orbit 2 */}
            <div className={"absolute inset-[30px] rounded-full border " + (dk ? "border-pink-500/7" : "border-pink-300/20")} style={{animation:"chSpin 16s linear infinite reverse"}}>
              <div className="absolute -bottom-1 right-[22%] w-2 h-2 rounded-full bg-pink-400 shadow-lg shadow-pink-400/40" />
            </div>
            {/* Orbit 3 */}
            <div className={"absolute inset-[58px] rounded-full border " + (dk ? "border-amber-500/5" : "border-amber-300/15")} style={{animation:"chSpin 11s linear infinite"}}>
              <div className="absolute top-[22%] -left-[3px] w-[7px] h-[7px] rounded-full bg-amber-400 shadow-lg shadow-amber-400/30" />
            </div>
            {/* Center heart */}
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-[78px] h-[78px] bg-gradient-to-br from-rose-500 to-pink-500 rounded-[22px] flex items-center justify-center text-[40px] shadow-2xl shadow-rose-500/25" style={{animation:"chHeartPulse 2.8s ease-in-out infinite"}}>
                💕
              </div>
            </div>
          </div>
        ...
        <div className="px-7 pb-12 sm:pb-16 sm:max-w-md sm:mx-auto sm:w-full">
          ...
          <Link href="/signup" className={"block w-full py-[18px] rounded-2xl text-center font-semibold text-[16px] transition-all active:scale-[0.98] border no-underline mb-5 " + (dk ? "bg-white/4 border-white/8 text-white hover:bg-white/8" : "bg-gray-50 border-gray-200 text-gray-700 hover:bg-gray-100")}>
            Create Account
          </Link>

          <p className={"text-center text-[11px] leading-relaxed " + (dk ? "text-white/18" : "text-gray-300")}>
