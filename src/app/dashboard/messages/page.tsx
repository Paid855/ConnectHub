            ...
            return (
              <div key={msg.id}>
                ...
                <div className={"flex " + (isMine ? "justify-end" : "justify-start")}
                  onContextMenu={(e) => { e.preventDefault(); if (!isDeleted) { setShowMenu(showMenu === msg.id ? null : msg.id); if (showReactions && showReactions !== msg.id) setShowReactions(null); } }}
                  onTouchStart={(e) => {
                    const timer = setTimeout(() => { if (!isDeleted) { setShowMenu(showMenu === msg.id ? null : msg.id); if (showReactions && showReactions !== msg.id) setShowReactions(null); } }, 500);
                    (e.currentTarget as any)._holdTimer = timer;
                  }}
                  onTouchEnd={(e) => { clearTimeout((e.currentTarget as any)._holdTimer); }}
                  onTouchMove={(e) => { clearTimeout((e.currentTarget as any)._holdTimer); }}
                >
                  <div className={"relative max-w-[75%] rounded-2xl px-3.5 py-2 " + (isDeleted ? (dc ? "bg-gray-800 border border-gray-700" : "bg-gray-100 border border-gray-200") : isMine ? "bg-gradient-to-br from-rose-500 to-pink-500 text-white" : (dc ? "bg-gray-800 text-white" : "bg-white text-gray-800 shadow-sm border border-gray-100"))}>
                    ...
                    {imgSrc && (
                      <div className="relative group cursor-pointer" onClick={(e) => { e.stopPropagation(); setShowMenu(null); setMediaViewer({ src: imgSrc, type: "image" }); }}>
                        <img src={imgSrc} alt="" className="max-w-full rounded-xl max-h-60 object-cover hover:brightness-90 transition-all" loading="lazy" />
                        <div className="absolute bottom-2 left-2"><span className="px-1.5 py-0.5 bg-black/50 backdrop-blur-sm rounded text-[9px] font-bold text-white/80">HD</span></div>
                    ...
                    {isVid && (
                      <div className="relative rounded-2xl overflow-hidden max-w-[260px]">
                        ...
                        <div className="absolute bottom-2 left-2">
                          <span className="px-1.5 py-0.5 bg-black/50 backdrop-blur-sm rounded text-[9px] font-bold text-white/80">HD</span>
                        </div>
                        <div className="absolute top-2 right-2 flex gap-1.5">
                          ...
                          {!isMine && mediaPrefs.showDownload && <a href={msg.content.replace("[VID]", "")} download onClick={e => e.stopPropagation()} className="w-8 h-8 bg-black/50 rounded-full flex items-center justify-center text-white text-xs backdrop-blur-sm hover:bg-black/70 transition-all"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg></a>}
