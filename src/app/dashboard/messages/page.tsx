...
import { Send, ArrowLeft, Shield, MessageCircle, Search, Heart, Smile, Phone, Video, Image as ImageIcon, X } from "lucide-react";
...
export default function MessagesPage() {
  ...
  const openChat = async (partner: Partner) => {
    setActivePartner(partner); setShowEmoji(false); setLimitHit(false);
  ...
  const handleImageSend = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (file.size > 3*1024*1024) { alert("Max 3MB for images"); return; }
      ...
      <div className={"flex-1 flex flex-col " + (activePartner ? "flex" : "hidden md:flex")}>
        {activePartner ? (<>
          ...
          <div className="flex items-center gap-3 p-4 border-b border-gray-100 bg-white">
            ...
            <div className="flex-1"><p className="font-bold text-gray-900 flex items-center gap-1">{activePartner.name} {activePartner.tier==="verified"&&<Shield className="w-4 h-4 text-blue-500"/>}</p><p className="text-xs text-emerald-500">Online</p></div>
            <div className="flex items-center gap-1">
              <button onClick={() => alert("Voice calls coming soon! This feature requires WebRTC integration.")} className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-emerald-100 hover:text-emerald-600 transition-all"><Phone className="w-4 h-4" /></button>
              <button onClick={() => alert("Video calls coming soon! This feature requires WebRTC integration.")} className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-blue-100 hover:text-blue-600 transition-all"><Video className="w-4 h-4" /></button>
            </div>
          </div>
