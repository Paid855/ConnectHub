...
import { Shield, Search, Heart, MessageCircle, Filter } from "lucide-react";
...
export default function BrowsePage() {
  ...
  const filtered = profiles.filter(p => {
    ...
    if (filter === "verified" && p.tier !== "verified") return false;
  ...
  return (
    <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((p, i) => (
            <div key={p.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-all group">
              ...
              <div className="p-4">
                <p className="text-xs text-gray-500 mb-2">{p.gender || "Member"}{p.lookingFor ? " · Looking for " + p.lookingFor : ""}</p>
                ...
                <div className="flex gap-2">
                  ...
                  <button className="w-10 h-10 border-2 border-rose-200 rounded-xl flex items-center justify-center text-rose-400 hover:bg-rose-50 transition-all"><Heart className="w-5 h-5" /></button>
                </div>
              </div>
            </div>
          ))}
      ...
      {!loading && filtered.length === 0 && <div className="text-center py-16"><p className="text-gray-500">No profiles match your search</p></div>}
    </div>
  );
}
