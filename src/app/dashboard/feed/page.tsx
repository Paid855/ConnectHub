...
export default function FeedPage() {
  ...
  const isVideo = (img: string|null) => img?.startsWith("[VID]") || img?.startsWith("data:video");
  const getMediaSrc = (img: string) => img.replace("[VID]","");

  const createPost = async () => {
    if (!newPost.trim() && !newImage && !newVideo) return;
    setPosting(true);
    const mediaData = newVideo ? "[VID]" + newVideo : newImage;
    await fetch("/api/feed", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ content:newPost, image:mediaData }) });
    setNewPost(""); setNewImage(null); setNewVideo(null); setShowEmoji(false); await loadFeed(); setPosting(false);
  };
  ...
  const reactToPost = async (postId: string, emoji: string) => {
    ...
    setShowReactions(null); await loadFeed();
  };
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => { setNewImage(ev.target?.result as string); setNewVideo(null); };
  ...
  const handleVideoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const reader = new FileReader();
    reader.onload = (ev) => { setNewVideo(ev.target?.result as string); setNewImage(null); };
    reader.readAsDataURL(file);
  };
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6">
        <div className="flex gap-3">
          ...
          <div className="flex-1">
            ...
            {newVideo && <div className="relative mt-2 inline-block"><video src={newVideo} className="max-h-48 rounded-xl border border-gray-200" controls /><button onClick={()=>setNewVideo(null)} className="absolute top-2 right-2 w-6 h-6 bg-black/50 rounded-full flex items-center justify-center"><X className="w-3 h-3 text-white" /></button></div>}
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
          <div className="flex gap-2">
            ...
            <button onClick={()=>videoInputRef.current?.click()} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-500 hover:bg-gray-100 rounded-lg"><Film className="w-4 h-4 text-violet-500" /> Video</button>
          ...
          {posts.map(post => (
            <div key={post.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              {post.image && isVideo(post.image) ? (
                <div className="bg-black"><video src={getMediaSrc(post.image)} controls preload="metadata" className="w-full max-h-[500px]" playsInline /></div>
              ) : post.image ? (
              ...
              <div className="relative flex border-t border-gray-100">
                {showReactions===post.id && (
                  <div className="absolute -top-12 left-4 bg-white rounded-full shadow-xl border border-gray-200 px-2 py-1.5 flex gap-1 z-20">
                    {REACTIONS.map(r => (<button key={r.key} onClick={()=>reactToPost(post.id,r.key)} className={"w-9 h-9 flex items-center justify-center text-xl hover:scale-125 transition-transform rounded-full "+(post.myEmoji===r.key?"bg-rose-100":"hover:bg-gray-100")}>{r.emoji}</button>))}
                  </div>
                )}
                <button onClick={()=>post.liked?reactToPost(post.id,post.myEmoji||"heart"):setShowReactions(showReactions===post.id?null:post.id)} className={"flex-1 flex items-center justify-center gap-2 py-3 text-sm font-medium transition-all "+(post.liked?"text-rose-500":"text-gray-500 hover:bg-gray-50")}>
                  {post.liked ? (<>{REACTIONS.find(r=>r.key===post.myEmoji)?.emoji||"❤️"} {post.myEmoji==="laugh"?"Haha":post.myEmoji==="love"?"Love":post.myEmoji==="wow"?"Wow":post.myEmoji==="sad"?"Sad":post.myEmoji==="fire"?"Fire":"Liked"}</>) : (<><Heart className="w-5 h-5"/> Like</>)}
                </button>
