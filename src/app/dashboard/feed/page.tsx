...
import { Heart, MessageCircle, Send, Image as ImageIcon, Shield, Smile, X, MoreHorizontal, Trash2, Crown, Gem } from "lucide-react";
...
type Post = { id:string; userId:string; content:string|null; image:string|null; createdAt:string; user:PostUser; likeCount:number; liked:boolean; comments:Comment[]; commentCount:number; };
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6">
        <div className="flex gap-3">
          ...
          <div className="flex-1">
            ...
            {showEmoji && <div className="flex flex-wrap gap-1 mt-2 p-2 bg-gray-50 rounded-xl">{EMOJIS.map(e => <button key={e} onClick={() => setNewPost(p => p+e)} className="w-8 h-8 flex items-center justify-center text-lg hover:bg-gray-200 rounded-lg">{e}</button>)}</div>}
          </div>
        </div>
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
          ...
          <button onClick={createPost} disabled={(!newPost.trim()&&!newImage)||posting} className="px-5 py-2 bg-gradient-to-r from-rose-500 to-pink-500 text-white rounded-full text-sm font-semibold hover:shadow-lg disabled:opacity-40 flex items-center gap-1.5">
          ...
          {posts.map(post => (
            <div key={post.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="flex items-center gap-3 p-4 pb-2">
                ...
                <div className="flex-1">
                  <Link href={"/dashboard/user?id=" + post.userId} className="text-sm font-bold text-gray-900 hover:text-rose-500 transition-colors flex items-center gap-1.5">
                    {post.user?.name}
                  ...
                  <div className="relative">
                    ...
                    {openMenu === post.id && (
                      <div className="absolute right-0 top-full mt-1 bg-white rounded-xl shadow-lg border border-gray-100 py-1 w-36 z-10">
                        <button onClick={() => deletePost(post.id)} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-500 hover:bg-red-50"><Trash2 className="w-4 h-4" /> Delete Post</button>
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="flex items-center justify-between px-4 py-2 text-xs text-gray-500">
                <span>{post.likeCount > 0 ? "❤️ " + post.likeCount : ""}</span>
                ...
                <div className="border-t border-gray-100 bg-gray-50 p-4">
                  {post.comments.map(c => (
                    <div key={c.id} className="flex gap-2.5 mb-3 last:mb-0">
                      <Link href={"/dashboard/user?id="+c.user?.id}>
