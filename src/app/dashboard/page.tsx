...
import { Heart, X, Star, MapPin, Shield, Sparkles, MessageCircle, Users, Search } from "lucide-react";
import { useUser } from "./layout";
import { useRouter } from "next/navigation";

type Profile = { id:string; name:string; age:number|null; gender:string|null; lookingFor:string|null; bio:string|null; profilePhoto:string|null; tier:string; verified:boolean; };
...
export default function DiscoverPage() {
  ...
  if (profiles.length === 0) return (
    <div className="text-center py-20">
      <div className="w-20 h-20 rounded-full bg-rose-100 flex items-center justify-center mx-auto mb-5"><Users className="w-10 h-10 text-rose-400" /></div>
      <h2 className="text-2xl font-bold text-gray-900 mb-2">No Profiles Yet</h2>
      <p className="text-gray-500 max-w-sm mx-auto">Be the first! Invite friends to join ConnectHub and start matching.</p>
    </div>
  );

  if (!p) return <div className="text-center py-20"><h2 className="text-2xl font-bold text-gray-900 mb-2">No more profiles</h2><p className="text-gray-500">Check back later for new people!</p></div>;
      ...
      <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="relative h-72">
              {p.profilePhoto ? (
                ...
                <div className={"w-full h-full bg-gradient-to-br " + gradients[idx % gradients.length] + " flex items-center justify-center"}>
                  <span className="text-7xl font-bold text-white/30">{p.name[0]}</span>
                </div>
              )}
              ...
              <div className="absolute bottom-3 left-5 flex items-center gap-2">
                ...
                {p.tier === "verified" && <Shield className="w-5 h-5 text-blue-500 fill-blue-100" />}
              </div>
              {p.tier === "verified" && <div className="absolute top-4 right-4 bg-blue-500 text-white text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1"><Shield className="w-3 h-3" />Verified</div>}
            </div>
            <div className="p-5">
              {p.gender && <p className="text-sm text-gray-500 mb-2">{p.gender}{p.lookingFor ? " · Looking for " + p.lookingFor : ""}</p>}
              ...
              <p className="text-sm text-gray-600 leading-relaxed mb-4">{p.bio || "No bio yet — say hello and find out more!"}</p>
            </div>
          </motion.div>
        ...
        <div className="lg:w-72 space-y-4">
            ...
            <div className="space-y-3">
              {profiles.slice(0, 6).map((pr, i) => (
                <div key={pr.id} className="flex items-center gap-3 py-1.5">
                  ...
                  <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-gray-900 truncate">{pr.name}{pr.age ? ", " + pr.age : ""}</p><p className="text-xs text-gray-400">{pr.gender || "Member"}</p></div>
          ...
          <div className="bg-gradient-to-br from-rose-500 to-pink-500 rounded-2xl p-5 text-white">
            ...
            <button className="w-full py-2 bg-white text-rose-500 rounded-xl text-sm font-bold hover:bg-rose-50 transition-all">Upgrade Now</button>
          </div>
        </div>
      </div>
