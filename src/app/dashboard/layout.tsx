            ...
            <div className={"absolute right-4 top-16 lg:right-6 lg:top-14 w-80 rounded-2xl shadow-2xl border overflow-hidden " + (dc?"bg-gray-800 border-gray-700":"bg-white border-gray-200")} onClick={e => e.stopPropagation()}>
              ...
              <div className="max-h-80 overflow-y-auto">
                {notifications.length === 0 ? (
                  ...
                  <div key={n.id} onClick={() => { setShowNotif(false); const link = n.type==="message"?"/dashboard/messages"+(n.fromUserId?"?user="+n.fromUserId:""):n.type==="friend_request"||n.type==="friend_accepted"?"/dashboard/friends":n.type==="like"||n.type==="superlike"?"/dashboard/liked":n.type==="gift"?"/dashboard/wallet":n.type==="verification"?"/dashboard/verify":n.type==="story_reply"||n.type==="story_react"?"/dashboard/messages":n.type==="purchase"||n.type==="boost"?"/dashboard/wallet":n.type==="purchase_failed"?"/dashboard/coins":n.type==="match"?"/dashboard/messages":"/dashboard"; router.push(link); if(!n.read){setNotifCount(p=>Math.max(0,p-1));setNotifications(p=>p.map(x=>x.id===n.id?{...x,read:true}:x));} fetch("/api/notifications",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"read",notificationId:n.id})}); }} className={"flex items-start gap-3 p-3 border-b last:border-0 cursor-pointer transition-colors " + (!n.read?(dc?"bg-rose-500/10 border-gray-700 hover:bg-gray-700":"bg-rose-50/50 border-gray-50 hover:bg-gray-50"):(dc?"border-gray-700 hover:bg-gray-700":"border-gray-50 hover:bg-gray-50"))}>
